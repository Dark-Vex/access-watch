...
function window({
  ...
  return stream => {
    return event => {
      ...
      statsd.set('pipeline.window.delta', t - watermark);
...
class Pipeline extends Builder {
    ...
    this.inputs.map(function(input, idx) {
      input.start({
        success: function(log) {
          ...
          statsd.timing('pipeline.validate', process.hrtime(validateStart)[1] / 1000000);
          if (valid) {
            statsd.increment('pipeline.valid');
            const event = Map({
              time: Math.floor(
            ...
            statsd.increment('pipeline.invalid');
  ...
  handleEvent(event) {
    statsd.set('pipeline.event.delta', event.get('time') - now())
